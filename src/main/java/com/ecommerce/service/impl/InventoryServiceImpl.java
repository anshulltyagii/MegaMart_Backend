...
public class InventoryServiceImpl implements InventoryService {
    ...
    // ════════════════════════════════════════════════════════════════════
    // ✅ UPDATED: Added Backend Console Logging for Stock Updates
    // ════════════════════════════════════════════════════════════════════
	@Override
	@Transactional
	public InventoryResponse createOrInitInventory(Long productId, int quantity) {
		if (quantity < 0) throw new BadRequestException("Initial quantity cannot be negative");
		...
		if (existing == null) {
			...
			// Reset reserved only if initializing, logic specific decision
    ...
    private void notifyShopkeeperOfLowStock(Long productId, int requestedQty) {
        try {
            ...
            emailReq.setSubject("⚠️ URGENT: Low Stock Alert for " + product.getName());
            emailReq.setMessage("Hello Shopkeeper,\n\nA customer attempted to buy " + requestedQty + 
                                " units of '" + product.getName() + "' (SKU: " + product.getSku() + 
	...
	private InventoryResponse mapToResponse(Inventory inv) {
		...
		if (available <= 0) resp.setStockStatus("OUT OF STOCK");
		else if (available <= 5) resp.setStockStatus("ONLY FEW LEFT");
		else resp.setStockStatus("IN STOCK");
		return resp;
	}
}